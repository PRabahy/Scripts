...
Func search($searchHandle, $searchLocation, $depth = -1)
	$toReturn = ""
	While (True)
		If $terminate Then
			$toReturn = 0
		...
		If StringInStr($attrib, "D") And Not $depth = 0 Then
			$depth -= 1
			$search2 = FileFindFirstFile($searchLocation & "\" & $file & "\*.*")
			$toReturn = $toReturn & search($search2, $searchLocation & "\" & $file, $depth)
		EndIf
